...
fused_k_30_rate  <- ggplot(data = drug_overdose_plot_sol) +
  geom_polygon(aes(x = long, y = lat,
  ...
  annotate(geom="label", x=-75, y=42, label=TeX("$\\hat{C}_1$"),
           fill="black", colour="white",size=5)+
  annotate(geom="label", x=-82, y=42, label=TeX("$\\hat{C}_5$"),
           fill="black", colour="white",size=3)+
  annotate(geom="label", x=-81.5, y=38.5, label=TeX("$\\hat{C}_2$"),
           fill="black", colour="white",size=3)+
  guides(fill = guide_colourbar(barwidth = 15, barheight = 1))+
...
p_ci_drug <- ggplot(data=result_list_df_ci_plot, aes(x=as.factor(x_plot), y=test_stat, colour=type))+
