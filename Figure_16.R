...
drug_overdose <- read.csv(paste0(input_dir,'Drug Overdose Mortality by State.csv'))
drug_overdose <- drug_overdose %>%
  dplyr::filter(YEAR==2018) %>%
  dplyr::select(c(STATE,RATE))%>%
  mutate(log_rate=log(RATE))

colnames(drug_overdose) <- c('states','rate','log_rate')
...
drug_overdose_plot <- drug_overdose %>%
...
#### observed rate
obs_rate_drug <- ggplot(data = drug_overdose_plot) +
...
####
K <- 27
log_rate <- drug_overdose$log_rate
# aux function
...
gfl_sol_staes <- data.frame(
  log_rate = drug_overdose$log_rate,
  State = drug_overdose$State,
  sol_step_K = GFL_sol$beta[,K]
)


drug_overdose_plot_sol <- drug_overdose_plot %>%
  inner_join(gfl_sol_staes, by = 'State')

fused_k_20_rate  <- ggplot(data = drug_overdose_plot_sol) +
  geom_polygon(aes(x = long, y = lat,
  ...
  labs(fill='Death Rate')+
  guides(fill = guide_colourbar())+
  annotate(geom="label", x=-110, y=40, label=TeX("$\\hat{C}_3$"),
           fill="black", colour="white",size=7)+
  annotate(geom="label", x=-82, y=32, label=TeX("$\\hat{C}_4$"),
           fill="black", colour="white",size=5)+
  annotate(geom="label", x=-75, y=42, label=TeX("$\\hat{C}_1$"),
           fill="black", colour="white",size=5)+
  annotate(geom="label", x=-84.5, y=38.5, label=TeX("$\\hat{C}_2$"),
           fill="black", colour="white",size=5)+
  guides(fill = guide_colourbar(barwidth = 15, barheight = 1))+
...
png(paste0(plot_output_dir,'Figure_16b.png'),
    width = 6,height=4,res=300,units='in')
print(fused_k_20_rate)
dev.off()
...
list_results <- list(p_val_c1_c2, p_val_c1_c3, 
  p_val_c1_c4, p_val_c2_c3,
  p_val_c2_c4, p_val_c3_c4)

result_list_df <-  dplyr::bind_rows(lapply(list_results,function(x)aggr_ci_info(x)))
result_list_df$x_plot <- c(1:length(list_results))
...
### create x axis ticks 
all_comb_names <- t(utils::combn(4, 2))
tick_labels <- (paste0("$\\bar{\\beta}_{",all_comb_names[,1],"}","-",
                       "\\bar{\\beta}_{",all_comb_names[,2],"}$"))

p_ci_drug <- ggplot(data=result_list_df_ci_plot %>% 
                      mutate(type=factor(result_list_df_ci_plot$type,
                                         levels = c("naive","union","hyun"))), 
                    aes(x=as.factor(x_plot), y=test_stat, colour=type))+
  geom_point(position = position_dodge(width = 0.5))+
  geom_pointrange(aes(ymin = lcb, ymax = ucb),size=0.7, fatten=0.5,
  ...
  scale_x_discrete(breaks=c(1:6), labels=unname(TeX(tick_labels)))

png(paste0(plot_output_dir,'Figure_16d.png'),
    width = 9,height=3,res=250,units='in')
print(p_ci_drug)
dev.off()
